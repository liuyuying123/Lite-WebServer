...
WebServer::WebServer(int port, int trigMode, int timeoutMS, bool optLinger,
                     ...
                     const char *dbName, int connPoolNum, int threadNum, int taskNum,
                     bool openLog, int logLevel, int logQueSize) :
        port_(port), openLinger_(optLinger), timeoutMS_(timeoutMS), isClose_(false),
        timer_(new HeapTimer()), threadPool_(new ThreadPool(threadNum,taskNum)), epoller_(new Epoller()) {
    /*获取资源目录*/
    ...
    /*源代码有错，初始化应该在日志初始化之后，不然初始化sql连接池时的错误都发现不了*/
...
void WebServer::dealRead_(HttpConn *client) {
    assert(client);
    extentTime_(client);
    if(!threadPool_->addTask(std::bind(&WebServer::onRead_, this, client))){/*将参数绑定到成员函数上，this是成员函数所属对象的指针，将client连接作为onRead_的参数*/
        /*在这里应该处理加新线程，之后再实现*/
        /*如果这么改了的话，那此socket本次的可读事件就被忽略了，我们重新注册它上面的读事件和EPOLLONESHOT事件来在下一轮再处理它,也不知道这种方案是否正确*/
        epoller_->modFd(client->getFd(), connEvent_ | EPOLLIN);
        LOG_INFO("task queue is full, add a thread!");
    }
    else{
        LOG_INFO("add a new task in task queue!");
    }
}
...
void WebServer::dealWrite_(HttpConn *client) {
    ...
    if(!threadPool_->addTask(std::bind(&WebServer::onWrite_, this, client))){
        /*任务队列满，重新注册socket上的写事件，下一轮再加入任务队列当中*/
        epoller_->modFd(client->getFd(),connEvent_ | EPOLLOUT);
        LOG_INFO("task queue is full, add a thread!");
    }
    else{
